...
// 参数：
//  - deps: { draftApi, activeProvider, activeModel, api, ctx, ui, chat, readFocusPaths, pushEvent, onReviewPaused? }
...
export function useVCardTurnApi({ draftApi, activeProvider, activeModel, api, ctx, ui, chat, readFocusPaths, pushEvent, onReviewPaused }) {
  const userInput = ref("");
  ...
  function appendLocalUserMessage({ requestId, userText }) {
    ...
    if (!rid || !text) return;
    const msg = { role: "user", content: text, time: nowTime(), localRequestId: rid, localStatus: LOCAL_STATUS.SENDING, localError: "" };
  ...
  function markLocalUserMessageFailed({ requestId, errorText }) {
    const m = findLocalUserMessageByRequestId(requestId);
    if (!m) return;
  ...
  function toTurnErrorText(err) {
    const code = String(err?.code || "").trim();
    if (code === "VERSION_CONFLICT") return buildRetryHintForVersionConflict(err);
    if (code === "DRAFT_BUSY") return "草稿存在进行中的 Run：请先停止 Run 再重试。";
    return `请求异常：${String(err?.message || err)}`;
  }
  ...
  async function runTurnsWithFirst({ first }) {
    ...
    try {
      ...
      draftApi.boardError.value = toTurnErrorText(err);
  ...
  async function applyPausedFinal(out) {
    ...
    else if (reason === "patch_review") draftApi.boardError.value = "已生成待确认变更（patch_review）：请在右侧 Pending 面板进行 Accept/Reject/追问。";
  ...
  function recordSendFailure(err) {
    ...
    const errorText = toTurnErrorText(err);
    ...
    markLocalUserMessageFailed({ requestId: reqId, errorText });
  }
