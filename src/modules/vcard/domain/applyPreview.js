 ...
 * 文件：applyPreview.js
 * 模块：vcard/domain
 * 作用：生成“已应用输出”的 Before/After 预览（用于 UI 直接对比）
 * 依赖：无
 * @created 2026-01-02
 * @modified 2026-01-02
 */
...
function pickChangedEntryIndices(changedPaths) {
  ...
  return Array.from(out).filter((n) => Number.isFinite(n) && n >= 0).sort((a, b) => a - b);
}
...
export function buildVcardApplyPreview({ beforeDraft, afterDraft, changedPaths }) {
  ...
  const worldbookEntryDiffs = byId.ok
    ? byId.diffs
    : buildWorldbookEntryDiffsByChangedPaths({ beforeEntries, afterEntries, changedPaths, limit: 12 });
