...
function markerToMessages(identifier, { preset, ctx, chat, vars }) {
  const sys = (text) =>
    [{ role: "system", content: renderTemplate(text, vars).trim() }].filter((m) => String(m?.content || "").trim());
 ...
 * 作用：按 SillyTavern prompts/prompt_order 生成 messages[]（支持 marker 注入）
 * 约束：只实现本项目需要的 marker；不做 token 截断；不处理 tool/multi-modal
 * 参数：
 *  - preset: object（对话补齐预设）
 *  - ctx: object（上下文：user/char/世界书片段等）
 *  - chat: {role:'user'|'assistant',content:string,noQuote?:boolean}[]（历史）
...
export function buildMessages({ preset, ctx, chat, userInput, useSystemPrompt }) {
  ...
  const hasChatHistoryMarker = order.some(
    (x) => x?.identifier === "chatHistory" && (x?.enabled === undefined ? true : Boolean(x.enabled)) && promptById.has("chatHistory"),
  );
